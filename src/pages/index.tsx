...
export default function TasksPage() {
  ...
  const handleComplete = async (task: Task) => {
    try {
      // Obtenemos los IDs de los trabajadores asignados
      const workerIds = task.workers.map(w => w.id);

      const res = await fetch('/api/tasks/edit', {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          // Enviamos todos los campos obligatorios, incluyendo los IDs de los trabajadores
          ...task,
          workerIds: workerIds,
