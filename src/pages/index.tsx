...
export default function Home() {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        ...
        setError(err.message || 'Ocurrió un error inesperado al obtener datos.');
      } finally {
        setIsLoading(false);
      }
    };
  ...
  // Aquí está la corrección: añadimos la verificación para los dos contadores
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
              {(role === 'supervisor' || role === 'admin') && (
                <div className="bg-white rounded-lg shadow-md p-6 border-l-4 border-yellow-500">
                  ...
                  <Link href="/leave" className="text-sm font-medium text-yellow-600 hover:text-yellow-800 mt-2 inline-block">Gestionar Solicitudes
                  </Link>
                </div>
              )}
              <div className="bg-white rounded-lg shadow-md p-6 border-l-4 border-blue-500">
                ...
                <Link href="/tasks" className="text-sm font-medium text-blue-600 hover:text-blue-800 mt-2 inline-block">Ver Tareas
                </Link>
              </div>
            </div>
          </>
